...
### 前言：
* normphp项目目前为本人一个人维护开发维护
    * 因精力有限README.md形式文档不再继续维护，后期将主要用来简单介绍引导各位了解normphp项目
    * 详细文档与项目生态介绍看访问normphp项目官网
* 关于官网
    * 官网地址 https://normphp.org
    * 因官网域名未在中国大陆进行ICP备案（org域名貌似无法备案）故官网服务运行在海外服务器上
    * 考虑到必要的安全问题，官网全站使用cloudflare提供的cdn代理服务（在此感谢cloudflare），因此官网在国际网络高峰期时（一般为晚上七点钟开始）访问速度缓慢请各位谅解。
* 关于项目的未来与初衷
    * 由于设想的normphp项目生态过于庞大，项目之前只是应用在个人项目中使用（代码全在个人gitlab中）于2021-01才慢慢的进行开源
    * normphp项目生态是本人在长期管理开发团队、维护生产环境、设计项目系统架构时碰到的各种问题而针对性规划设计的项目
        * 会基础PHP、LNMP环境知识即可开箱即用运行
        * 开发生态包需要一定的 composer packagist 开发规范知识
          * 控制器、Model（表结构的定义）等都在composer包中开发，在框架初始化时会自动扫描vendor目录下包中的控制器，自动化在app目录创建继承包中控制器的门面控制器对外提供服务。
          * 这样有效的解决了PHP项目模块的快速迭代、多项目的模块复用问题（传统开发模式是在项目中直接编写业务代码、如其他项目或者微服务也需要此业务代码逻辑时就需要人工手动复制备份代码，非常不利于模块化微服务化开发的开发模式）
          * 这样的开发模式非常适合业务模块化、微服务化开发
        * 正如项目名normphp：norm为规范|标准之意，项目生态包在开发时有着较强的规范性（路由的定义、请求参数的数据结构定义、路由返回数据的数据结构定义等）
### normphp特性简介：
* 强大的注解来规范
